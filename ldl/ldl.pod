...
Well first off you'd have to be able to see well enough to use a graphical CAD-like program - a level/map editor - to draw the shapes of your rooms on the screen.

=item 2.

The shapes you draw - brushes - would make up the rooms in the map.  It works like building blocks and you have to put many blocks together to make a single room - one for the floor, ceiling and all of the sides.  If you wanted to put a hole in a wall (for a door, for example), you would have to build the brushes around the space where you wanted the hole to be.
...
Textures would have to be applied to the brushes.  Entities (weapons, lights, monsters) would be added to the map to make it more interesting.  Doors and buttons are special entities that are associated with a brush (that moves, for example, when you walk near it).

=item 4.

When the map is finished it must be saved as a F<.map> file so that the level compilation tools can read it.  The F<.map> file is even lower-level than many level editors' internal formats.  It contains a list of the brushes and entities in the map, but with the bushes defined in terms of triples of points on the 3D planes that make them up.

=item 5.

Finally the F<.map> file can be compiled with the Quake map compilation tools.  These would be run as follows.
...
LDL - the code - is actually a chain as mentioned above.  Each stage is a separate program and the whole thing works by feeding the output of one into the next, until we have our final answer - the F<.map> file.
...
Write your XML file with a text editor, as per the examples in the tutorial section below.

=item 2.

Run the LDL tools on it (a script/batch file is provided to automate this).  They should be run from a terminal/command window.
...
=head2 What Does the LDL Test Release Do?
...
=head2 Supported, Planned and Blue Sky Features
...
For now, however, there's a great deal you I<can> do - as we hope the tutorial below will show.
...
or similar.  On Windows, you should get the Windows Installer package from L<http://python.org/ftp/python/2.5.2/python-2.5.2.msi> and install it.

=head2 Getting LDL

The test release is available from L<http://www.agrip.org.uk/ldl/>.  It contains the LDL map translation scripts and Quake map compilation tools.
...
There are no restrictions on where you should unpack the zip file and run the scripts from.  However, you'll need to copy the Quake map compilation tools (F<qbsp>, F<light> and F<vis>) to somewhere on your path (such as /usr/local/bin/ or ~/bin/ if it's on your path).
...
When you've installed Python 2.5 and downloaded the test release, please unzip it into your AudioQuake directory, so that the F<ldl/> folder is inside your AudioQuake folder, alongside F<id1/>. 
...
=head3 Expected Test Results

This should (a) transform the XML into a F<.map> file using the LDL scripts, then (b) compile the map using the quake map tools and (c) run it using AudioQuake.

If this is not what happens, consult L<http://www.agrip.org.uk/ldl/> and the AGRIP-discuss mailing list for help - L<http://lists.agrip.org.uk/cgi-bin/mailman/listinfo/agrip-discuss>.  Any error messages will be output to the command window for you to read and submit as part of your email to the mailing list.  If you are working on your own map, it's also a good idea to send in your XML file so we can see if there are any errors in it.


=head1 Making Maps with LDL
...
LDL maps are XML files (which are plain text files and look similar to HTML files).  You can edit them with any text editor.  You should be able to copy and paste from this document into a text file - name it something like F<tutorial.xml> and run it through the LDL system in a terminal/command window, as above.

If you are new to XML don't worry - you can copy the examples given here.  Also, there is a tutorial on XML at L<http://xml.resource.org/authoring/draft-mrose-writing-rfcs.html#xml_basics>.

=item *

When you run the LDL scripts on a map, you should not include the F<.xml> part of the map's name.
...
This starts our map off; it's an XML I<element> called "map".  Elements are written inside angle-brackets and may also have additional information specified about them by way of I<attributes>.  The map element has two attributes:
...
A I<style>, which is "base" in this case.  This affects lighting, texturing, sounds and key types used in the map.
...
As well as having attributes, an element can contain other elements.  When we get to the end of the map, we must I<close> the map element with </map>.
...
The room has been given an I<id> so that LDL can uniquely identify it later.  You must supply an ID for all rooms in your map.  We didn't bother to specify a size for it, so it will default to being "medium" in size in all 3 directions - width, depth and height.

 <item type='info_player_start' pos='c' />

This room contains one I<item> - a player start point which, according to its C<pos> attribute, is to be positioned in the centre of its containing room.  The item itself does not contain anything (this wouldn't make much sense) so it can be closed simply by writing a forward slash before the closing angle bracket.  This is the same as if we had written it out in full, as follows.
...
Try running this map through the LDL system.  As part of the conversion to a F<.map> file (which will be converted by the standard Quake mapping tools to a F<.bsp>) the level will be textured and lit appropriately for the style chosen above.  Save the XML as F<tut01.xml> and convert, compile and test the map with, if you're using Linux:
...
Remember to just type the part of the filename before the F<.xml> extension.  Once the LDL scripts have run and the Quake map tools have finished processing, the game will start and you should be in a medium-sized room with not much else going on.
...
We need to give each of these rooms a unique ID attribute, however, so we may as well name them partly after the compass points, as we'll be using those to place them around the outside of the central room.  So, our side room IDs will be "spawn_south", "spawn_north", "spawn_east" and "spawn_west".  We'll put our C<info_player_start> in the southern one, so it will look more like this:
...
We've got the basics of a deathmatch map covered, but before we compile it, we need to consider one last thing: the direction the player faces when spawning.  We have positioned spawn rooms around a central room.  By default the player will spawn facing north.  This is fine if they spawn in the southern room because they'd spawn facing the door that leads onto the main room.  However, when the player spawns in one of the other side rooms, they will be facing the wrong way.  To correct this, we can supply an C<angle> attribute for the spawn points.
...
=head3 Co-Operative Maps

These are similar to deathmatch maps in that you need to include more than one spawn point.  However, the spawn points are usually situated together at the "start" of the map and must be of type C<info_player_coop> (you can have as many as you like).
...
This is very similar to the example above (F<tut05>), however, we have added two attributes to the connection from the room "start" to the room "other".  These are:

=over

=item pos

This is the position I<on the connecting wall> that you want the hole/door to be created.  That is to say: when you specify a position - "top" (t) in this case - you are specifying the position on that wall where you want the hole to be made, as if you were standing in front of the wall itself.
...
If you'd like to try a plat instead of stairs, change the value of the C<elevtype> attribute for the connection to "plat" and try the map again.

=head3 Even More Positioning

Try changing the C<pos> to "tl" from "t" and you'll find that you have to move to the left/west to find the stairs up to room "other".

It is important to note that even when you use the C<pos> attribute to change the position on the wall in the "start" room, you still always end up in "other" at the same place - the connection on that side is made in the default place of the middle of the connecting wall, at ground level.
...
=head3 Making Connection Corridors
...
We've added an attribute, C<extent>, to the connection and given it the size "+".  That should be read as "fill all available space".  Try making this change and running the map - the stairs will take up all of the available space in the corridor but not above the height they're meant to be).

You can specify sizes ("small", "med" and so on) in the C<extent> attribute, as you can with rooms.  You can also mix and match, so saying C<extent='+ med'> means "take up all available width, but be only medium-sized in depth".  Bear in mind that if the corridor was going from east to west (or vice-verse) these would be reversed (which is why it's often easier to just use "+" as the size specifier).

=head3 Vertical Connections to Other Rooms
...
This has been a tutorial of the LDL system's concept release.  There is a lot more to do and a lot of feedback to collect on how it is going so far!  If you need more information, please consult L<http://www.agrip.org.uk/ldl/> and the AGRIP-discuss mailing list - L<http://lists.agrip.org.uk/cgi-bin/mailman/listinfo/agrip-discuss>.
...
Rooms must all have a unique C<id> attribute.
...
=head1 Reference Information

=head2 The Style File - Automating Texturing and Lighting
...
The file F<style.xml> included with LDL contains information on the textures and lighting.  It will be document more fully in the coming releases.

=head2 List of Valid Entities
...
=item ambient_comp_hum

Computer background sounds

=item ambient_flouro_buzz

Flourescent buzzing sound

=item ambient_light_buzz

Buzzing sound from light

=item ambient_suck_wind
...
=item ambient_thunder

Thunder sound

=item event_lightning
...
=item item_artifact_envirosuit

Environmental Protection Suit

=item item_artifact_super_damage

Quad Damage

=item item_artifact_invulnerability

Pentagram of Protection

=item item_artifact_invisibility
...
=item light_torch_small_walltorch

Small wall torch (gives off light)

=item light_flame_large_yellow

Large yellow fire (gives off light)

=item light_flame_small_yellow

Small yellow fire (gives off light)

=item light_flame_small_white
...
=item monster_enforcer

Enforcer

=item monster_hell_knight

Hell Knight

=item monster_shalrath

Shalrath

=item monster_tarbaby
...
Shubb-Niggurath (requires a misc_teleportrain and a info_intermission)
...
=item trigger_teleport

Teleport (all trigger_ tags are triggered by walkover)

=item trigger_changelevel

Changes to another level

=item trigger_setskill

Changes skill level

=item trigger_counter
...
=item trigger_multiple

Triggers action (can be retriggered)

=item trigger_onlyregistered
