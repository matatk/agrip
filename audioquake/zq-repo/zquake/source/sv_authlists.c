...
qbool SV_AuthListAdd(authqh_t *header)
{
    ...
    new = Q_malloc(sizeof(authclient_t));
    ...
    {
        Q_free(new);
        Com_Printf("SV_AuthListAdd: Malformed M2S_AUTH_TOK packet (missing name) from %s.\n", NET_AdrToString (net_from));
        return false;
    }
    ...
    if( !msgline )
    {
        Q_free(newname);
        Q_free(new);
        Com_Printf("SV_AuthListAdd: Malformed M2S_AUTH_TOK packet (missing hash) from %s.\n", NET_AdrToString (net_from));
        return false;
    }
    ...
    // Read, allocate and store client_addr...
    msgline = MSG_ReadStringLine();
    if( !msgline )
    {
        Q_free(newname);
        Q_free(new);
        Com_Printf("SV_AuthListAdd: Malformed M2S_AUTH_TOK packet (missing client address) from %s.\n", NET_AdrToString (net_from));
        return false;
    }
    if( !NET_StringToAdr(msgline, &new->client_addr) )
    {
        Q_free(newname);
        Q_free(new);
        Com_Printf("SV_AuthListAdd: Error determining client address in M2S_AUTH_TOK packet from %s.\n", NET_AdrToString (net_from));
        return false;
    }
...
qbool SV_AuthListValidate(authqh_t *header, authclient_t **qitem)
{
    ...
    *qitem = SV_AuthListFind(header, msgline);
    if( *qitem == NULL )
    {
...
void SV_AuthListPrint(authqh_t *header)
{
    ...
    while( temp != NULL )
    {
        Com_Printf("\t\tac: %s, %s, %i, %s\n", temp->name, temp->hash, temp->valid, NET_AdrToString(temp->client_addr));
        temp = temp->next;
    }
